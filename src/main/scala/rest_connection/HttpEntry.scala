...
trait Service extends Protocols with HttpRequester {
  ...
  val classify =
    path("classify") {
      (post & entity(as[ClassifyRequest])) { request =>
        ...
        logger.info(s"got single classify request for ${request.algorithm}")
        logger.info(s"with raw text: ${request.text}")
        ...
        complete {
          classifyResult.map[ToResponseMarshallable] {
            case ClassificationResult(rep, dem) =>
              logger.info(s"classified with ${request.algorithm}, " + "rep:" + rep + ", dem:" + dem)
  ...
  path("classifyBulk") {
    (post & entity(as[ClassifyBulk])) { request =>
      ...
      logger.info(s"got twitter classify request for ${request.texts.size} posts.")
